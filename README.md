...
- `/原图` 命令剥离 ImgBed 处理参数后重发未修改的原文件直链；会话历史未命中时可改用文件名直接到图床查原图
...
`/原图` 始终剥离 ImgBed 处理参数、发送未修改的原文件直链，不受 `image.sendMode` 影响。

找回顺序：

1. 先在**本会话的原图历史**（最近 30 张）中匹配：精确文件名 → 省略扩展名 → 唯一子串；多候选时列出提示请写更完整的文件名
2. 历史未命中时，按给出的文件名**直接到图床查原图**：探测为 404/410 才回复未找到（并附上尝试过的完整直链），403/429/网络异常等「说不准」的情形照常发送
3. 不带参数且本会话无历史时，回复用法提示

> 文件名直查要求已配置图床域名，且文件名需落在图床 `/file/` 目录下（可带子目录，如 `/原图 2026/09/abc.jpg`）。
